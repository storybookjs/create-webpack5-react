...
4. Optionally add a short, human-written summary of the change to
   `CHANGELOG.md`. (The older entries were generated by `auto`, which has been
   removed — don't try to reproduce that format; a plain bullet list per version
   is fine.)
